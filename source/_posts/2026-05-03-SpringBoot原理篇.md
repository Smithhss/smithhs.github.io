---
title: SpringBoot原理篇
date: 2026-05-03 13:48:40
tags:
  - Java
  - JavaWeb
  - 后端
categories: 编程开发
cover: 'https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230113144757856.png'
---
...
![image-20230113144757856](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230113144757856.png)
...
![image-20230113145158771](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230113145158771.png)
...
![image-20230113162746634](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230113162746634.png)

![image-20230113162639630](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230113162639630.png)

重启服务，同时配置Tomcat端口(三种配置文件、系统属性、命令行参数)，测试哪个Tomcat端口号生效：

![image-20230113165006550](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230113165006550.png)

删除命令行参数配置，重启SpringBoot服务：

![image-20230113170841253](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230113170841253.png)
...
![image-20230113172313655](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230113172313655.png)

![image-20230113172854016](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230113172854016.png)

运行jar程序：

- 同时设置Java系统属性和命令行参数

![image-20230113172659269](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230113172659269.png)

- 仅设置Java系统属性

![image-20230113173228232](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230113173228232.png)
...
通过Spring当中提供的注解@Component以及它的三个衍生注解（@Controller、@Service、@Repository）来声明IOC容器中的bean对象，同时我们也学习了如何为应用程序注入运行时所需要依赖的bean对象，也就是依赖注入DI。
...
![image-20230113211619818](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230113211619818.png)

> 问题：输出的bean对象地址值是一样的，说明IOC容器当中的bean对象有几个？
>
> 答案：只有一个。        （默认情况下，IOC中的**bean对象是单例**）
>
...
class DemoSpringbootApplicationTests {
    ...
    // 让springboot帮我们注入一个 ioc 容器对象,该容器中包含了所有的 bean
    @Autowired
    private ApplicationContext ioc;

    @Autowired
    private MyDruidData druidData;

    @Test
    void contextLoads() {
        // 面向 容器,获取指定的bean
        DeptController controller = (DeptController) ioc.getBean("deptController");
        DeptController controller2 = (DeptController) ioc.getBean("deptController");
        System.out.println(controller);
        System.out.println(controller2);

        DeptService bean = ioc.getBean(DeptService.class);
        DeptService bean2 = ioc.getBean(DeptService.class);
        System.out.println(bean);
        System.out.println(bean2);

        DeptDao dao = ioc.getBean("deptDao", DeptDao.class);
        DeptDao dao2 = ioc.getBean("deptDao", DeptDao.class);
    ...
    @Test
    void testBean3() {
        DeptController bean = ioc.getBean(DeptController.class);
        System.out.println(bean.getAbc());
        System.out.println(bean.getPort());
        System.out.println(bean.getMaxActive());
        String bean2 = ioc.getBean("myString",String.class);
    ...
    @Test
    void testAbc() {
        System.out.println(ioc.getBean(ABC.class));
    }
...
在前面我们提到的IOC容器当中，**默认bean对象是单例模式**(只有一个实例对象)。
...
![image-20230113214244144](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230113214244144.png)
...
![image-20230114001348839](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114001348839.png)
...
![image-20230114001736151](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114001736151.png)
...
![image-20230114003903285](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114003903285.png)
...
![image-20230114171304644](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114171304644.png)
...
![image-20230114170610438](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114170610438.png)
...
![image-20230114172442018](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114172442018.png)
...
![image-20230114173645101](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114173645101.png)
...
![image-20230114174805852](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114174805852.png)
...
![image-20230114205745221](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114205745221.png)

![image-20230114213945851](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114213945851.png)

![image-20230114212750007](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114212750007.png)
...
![image-20230114220159619](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114220159619.png)

> 所以配置类最终也是SpringIOC容器当中的一个bean对象

在IOC容器中除了我们自己定义的bean以外，还有很多配置类，这些配置类都是SpringBoot在启动的时候加载进来的配置类。这些配置类加载进来之后，它也会生成很多的bean对象。

![image-20230114221341811](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114221341811.png)
...
![image-20230114222245520](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114222245520.png)
...
![image-20230114224107653](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114224107653.png)
...
![image-20230114225018255](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114225018255.png)
...
![image-20230114231121016](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114231121016.png)
...
> 重新执行测试方法，控制台日志输出：
>
> ![image-20230114231709392](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114231709392.png)
...
> 执行测试方法：
>
> ![image-20230114233252259](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114233252259.png)
...
> 执行测试方法：
>
> ![image-20230114234222946](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114234222946.png)
...
![image-20230115001439110](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115001439110.png)
...
![image-20230115001950076](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115001950076.png)
...
![image-20230115002450993](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115002450993.png)
...
![image-20230115002743115](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115002743115.png)
...
>
> ![image-20230115003242549](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115003242549.png)

AutoConfigurationImportSelector类中重写了ImportSelector接口的selectImports()方法：

![image-20230115003348288](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115003348288.png)

> selectImports()方法底层调用getAutoConfigurationEntry()方法，获取可自动配置的配置类信息集合

![image-20230115003704385](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115003704385.png)

> getAutoConfigurationEntry()方法通过调用getCandidateConfigurations(annotationMetadata, attributes)方法获取在配置文件中配置的所有自动配置类的集合

![image-20230115003903302](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115003903302.png)
...
![image-20230129090835964](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230129090835964.png) 

![image-20230115064329460](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115064329460.png)
...
![image-20230115005159530](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115005159530.png)



第三方依赖中提供的GsonAutoConfiguration类：

![image-20230115005418900](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115005418900.png)

> 在GsonAutoConfiguration类上，添加了注解@AutoConfiguration，通过查看源码，可以明确：GsonAutoConfiguration类是一个配置。
>
> ![image-20230115065247287](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115065247287.png)
...
>
> ![image-20230115203748022](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115203748022.png)
>
...
> 执行testHeaderParser()测试方法：
>
> ![image-20230115211029855](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115211029855.png)
>
...
> 执行testHeaderParser()测试方法：
>
> ![image-20230115211351681](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115211351681.png)
>
...
> 执行testHeaderParser()测试方法：
>
> ![image-20230115211957191](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115211957191.png)
>
...
> 执行testHeaderParser()测试方法：
>
> ![image-20230115220235511](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115220235511.png)
...
![image-20230115222128740](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115222128740.png)

最后再给大家梳理一下自动配置原理：

![image-20230115222302753](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115222302753.png)
...
![image-20230115224939131](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115224939131.png)
...
![image-20230115225703863](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115225703863.png)
...
![image-20230115234739988](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115234739988.png)

![image-20230115234823134](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115234823134.png)

创建完starter模块后，删除多余的文件，最终保留内容如下：

![image-20230115235429353](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115235429353.png)
...
![image-20230116000302319](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230116000302319.png)

![image-20230115235921014](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230115235921014.png)

创建完starter模块后，删除多余的文件，最终保留内容如下：

![image-20230116000542905](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230116000542905.png)
...
![image-20230116001622679](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230116001622679.png)
...
![image-20230116002417105](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230116002417105.png)

![image-20230116002442736](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230116002442736.png)
...
>
> ![image-20230116002747681](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230116002747681.png)
>
> 删除AliOSSUtils类中的@Component注解、@Autowired注解
>
> ![image-20230116003046768](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230116003046768.png)



下面我们就要定义一个自动配置类了，在自动配置类当中来声明AliOSSUtils的bean对象。

![image-20230116003513900](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230116003513900.png)
...
![image-20230116004957697](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230116004957697.png)
...
![image-20230116005530815](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230116005530815.png)
...
![image-20230116011039611](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230116011039611.png)

用postman工具进行文件上传：

![image-20230116010731914](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230116010731914.png)

通过断点可以看到自动注入AliOSSUtils的bean对象：

![image-20230116011501201](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230116011501201.png)
...
![image-20230114180044897](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114180044897.png)
...
![image-20230114191737227](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114191737227.png)
...
![image-20230114192921673](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114192921673.png)



我们在学习这些web后端开发技术的时候，我们都是基于主流的SpringBoot进行整合使用的。而SpringBoot又是用来简化开发，提高开发效率的。像过滤器、拦截器、IOC、DI、AOP、事务管理等这些技术到底是哪个框架提供的核心功能？

![image-20230114193609782](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114193609782.png)
...
![image-20230114195143418](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230114195143418.png)
