---
title: 13-后端Web进阶(AOP)
date: 2026-03-19 20:05:54
tags:
  - Java
  - JavaWeb
  - 后端
categories: 编程开发
cover: 'https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311090750.png'
---
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311090750.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311090916.png)

而这个功能如果通过AOP来实现，我们只需要单独定义下面这一小段代码即可，不需要修改原始的任何业务方法即可记录每一个业务方法的执行耗时。

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311090938.png)

所以，AOP的优势主要体现在以下*四个方面*：
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311091657.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311092139.png)

我们可以看到，在控制台中输出了方法的执行耗时：

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311104238.png)
...
* **连接点：JoinPoint**，可以被AOP控制的方法（暗含方法执行时的相关信息）
  ...
  * 在SpringAOP提供的JoinPoint当中，封装了连接点方法在执行时的相关信息。（后面会有具体的讲解）
  ...
  ![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311104415.png)
...
* **通知：Advice**，指哪些重复的逻辑，也就是共性功能（最终体现为一个方法）
  ...
  * 是在AOP面向切面编程当中，我们只需要将这部分重复的代码逻辑抽取出来单独定义。抽取出来的这一部分重复的逻辑，也就是共性的功能。
  ...
  ![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311104514.png)
...
* **切入点：PointCut**，匹配连接点的条件，通知仅会在切入点方法执行时被应用。
  ...
  * 在aop的开发当中，我们通常会通过一个切入点表达式来描述切入点(后面会有详解)。
  ...
  ![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311104611.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311104710.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311104854.png)

AOP的核心概念我们介绍完毕之后，接下来我们再来分析一下我们所定义的通知是如何与目标对象结合在一起，对目标对象当中的方法进行功能增强的。

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311105140.png)

Spring的AOP底层是基于动态代理技术来实现的，也就是说在程序运行的时候，会自动的基于动态代理技术为目标对象生成一个对应的代理对象。在代理对象当中就会对目标对象当中的原始方法进行功能的增强。

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311105813.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311155015.png)

查看idea中控制台日志输出：

> 程序没有发生异常的情况下，@AfterThrowing标识的通知方法不会执行。

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311155333.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311155411.png)

查看idea中控制台日志输出

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311155359.png)

> 程序发生异常的情况下：
>
> * @AfterReturning标识的通知方法不会执行，@AfterThrowing标识的通知方法执行了  //互斥
>
...
怎么来解决这个切入点表达式重复的问题？ 答案就是：**抽取**   *Pointcut*
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311160850.png)

查看idea中控制台日志输出

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311160906.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311163724.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311164414.png)

  * @annotation(……) ：根据注解匹配

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311164428.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311184424.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311184524.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311185658.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311185212.png)

* 对于其他四种通知，获取连接点信息只能使用`JoinPoint`，它是`ProceedingJoinPoint`的父类型

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311185222.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311185019.png)
...
> ) comment '操作日志表';
>
> * 并且已经提供了OperateLogMapper接口来操作 operate\_log, 并在其中已经定义好了 insert 方法用来保存日志数据.
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311185154.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311185241.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311200140.png)

#### 3.6.2 记录当前登录员工

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311185315.png)
...
* 在`TokenFilter`中，解析完当前登录员工ID，将其存入ThreadLocal（用完之后需将其删除）。
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250311185335.png)
