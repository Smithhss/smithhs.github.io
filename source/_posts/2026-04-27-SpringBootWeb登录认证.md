...
tags:
  ...
  - 后端
categories: 编程开发
cover: 'https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230105085212629.png'
---
...
登录认证,实现的效果就是用户必须登录之后，才可以访问后台系统中的功能。

![image-20230105085212629](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230105085212629.png)

## 1. 登录功能

### 1.1 需求

![image-20230105085404855](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230105085404855.png)
...
![image-20230105175310401](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230105175310401.png)
...
@Override
    public String login(LoginDto dto) {
        /*
            1: 根据用户名和密码查询数据库  TODO 密码暂时不加密
            2: 判断用户名密码是否正确
            3: 登录成功,准备一个 map集合,保存想让令牌保存的信息,暂时存 id和name,生成令牌并返回
            4: 登录失败,返回 null
         */
        ...
        if(emp.getPassword().equals(dto.getPassword())){
            // 登录成功
            //准备一个 map集合,保存想让令牌保存的信息,暂时存 id和name,生成令牌并返回
            Map<String, Object> map = Map.of("id", emp.getId(), "name", emp.getName());
            // 使用生成令牌的工具类,生成令牌并返回
            String s = myJwtUtils.generateJwt(map);
            return s;
        }
        // 说明密码错误
        return null;
...
![image-20220907132229245](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20220907132229245.png) 

postman测试通过了，那接下来，我们就可以结合着前端工程进行联调测试。

先退出系统，进入到登录页面：

![image-20230105193104848](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230105193104848.png)
...
![image-20230105192918098](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230105192918098.png)
...
![image-20220907133329021](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20220907133329021.png)
...
![image-20230105180811717](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230105180811717.png)

什么是登录校验？

- 所谓登录校验，指的是我们在服务器端接收到浏览器发送过来的请求之后，首先我们要对请求进行校验。先要校验一下用户登录了没有，如果用户已经登录了，就直接执行对应的业务操作就可以了；
- 如果用户没有登录，此时就不允许他执行相关的业务操作，直接给前端响应一个错误的结果，最终跳转到登录页面，要求他登录成功之后，再来访问对应的数据。

接下来我们分析一下登录校验大概的实现思路。
...
![image-20230105194710533](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230105194710533.png)

那应该**怎么来实现登录校验的操作呢**？具体的实现思路可以分为两部分：
...
![image-20230105203827355](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230105203827355.png)

需要注意的是：**会话是和浏览器关联的，当有三个浏览器客户端和服务器建立了连接时，就会有三个会话**。同一个浏览器在未关闭之前请求了多次服务器，这多次请求是属于同一个会话。

比如：1、2、3这三个请求都是属于同一个会话。当我们关闭浏览器之后，这次会话就结束了。而如果我们是直接把web服务器关了，那么所有的会话就都结束了。
...
cookie 是客户端会话跟踪技术，**它是存储在客户端浏览器的**,在服务器端来设置一个cookie。
...
![image-20230112101901417](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112101901417.png) 
...
![image-20230112101804878](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112101804878.png)
...
![image-20230112105410076](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112105410076.png) 

我们可以看到，设置的cookie，通过**响应头Set-Cookie**响应给浏览器，并且浏览器会将Cookie，存储在浏览器端。

![image-20230112105538131](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112105538131.png) 



B. 访问c2接口 http://localhost:8080/c2，此时浏览器会自动的将Cookie携带到服务端，是通过**请求头Cookie**，携带的。

![image-20230112105658486](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112105658486.png) 
...
> 跨域介绍：
>
> <img src="https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112103840467.png" alt="image-20230112103840467" style="zoom:80%;" /> 
>
...
前面介绍的时候，我们提到Session，它是服务器端会话跟踪技术，所以它是存储在服务器端的。

- 获取Session

  ![image-20230112105938545](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112105938545.png) 
...
- 响应Cookie (JSESSIONID)
  ...
  ![image-20230112110441075](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112110441075.png) 
...
- 查找Session
  ...
  ![image-20230112101943835](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112101943835.png) 
...
![image-20230112111004447](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112111004447.png) 

请求完成之后，在响应头中，就会看到有一个Set-Cookie的响应头，里面响应回来了一个Cookie，就是JSESSIONID，这个就是服务端会话对象 Session 的ID。

B. 访问 s2 接口，http://localhost:8080/s2

![image-20230112111137207](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112111137207.png) 

接下来，在后续的每次请求时，都会将Cookie的值，携带到服务端，那服务端呢，接收到Cookie之后，会自动的根据JSESSIONID的值，找到对应的会话对象Session。

那经过这两步测试，大家也会看到，在控制台中输出如下日志：

![image-20230112111328117](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112111328117.png) 
...
>
> ​	<img src="https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112112557480.png" alt="image-20230112112557480" style="zoom:67%;" /> 
>
> - 首先第一点，我们现在所开发的项目，一般都不会只部署在一台服务器上，因为一台服务器会存在一个很大的问题，就是单点故障。所谓单点故障，指的就是一旦这台服务器挂了，整个应用都没法访问了。
>
> ​    ![image-20230112112740131](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112112740131.png) 
>
> - 所以在现在的企业项目开发当中，最终部署的时候都是以集群的形式来进行部署，也就是同一个项目它会部署多份。比如这个项目我们现在就部署了 3 份。
>
> - 而用户在访问的时候，到底访问这三台其中的哪一台？其实用户在访问的时候，他会访问一台前置的服务器，我们叫**负载均衡服务器**，它的作用就是将前端发起的请求均匀的分发给后面的这三台服务器。
>
>   ![image-20230112113558810](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112113558810.png) 
>
...
在现在的企业开发当中，基本上都会采用第三种方案，通过令牌技术来进行会话跟踪。

##### 2.2.2.3 方案三 - 令牌技术

这里我们所提到的令牌，其实它就是一个用户身份的标识，看似很高大上，很神秘，其实本质就是一个字符串。

![image-20230112102022634](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112102022634.png) 
...
- 定义了一种简洁的、自包含的格式，用于在通信双方以json数据格式安全的传输信息。由于数字签名的存在，这些信息是可靠的。
  ...
  >
  > 自包含：指的是jwt令牌，看似是一个**随机**的字符串，但是我们是可以根据自身的需求在jwt令牌中存储自定义的数据内容。如：可以直接在jwt令牌中存储用户的相关信息。
  >
...
![image-20230106085442076](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230106085442076.png)
...
![image-20230112114319773](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112114319773.png) 
...
![image-20230106190950305](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230106190950305.png)
...
![image-20230106205045658](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230106205045658.png)
...
通过JWT令牌技术来跟踪会话。具体的思路我们前面已经分析过了，主要就是两步操作：
...
# 自定义 jwt 令牌相关的信息
myjwt:
  key: sdjfkgsdsfgyusadgfklsay78657iusfdgu98dstfoghkjsdtgf
  time: 86400000
```

~~~java
// 这个类的对象创建,让spring完成,同时 使用 @value 注解读取配置文件中的信息
@Component
...
![image-20230106212805480](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230106212805480.png)

打开浏览器完成前后端联调操作：利用开发者工具，抓取一下网络请求

![image-20230106213419461](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230106213419461.png)
...
![image-20230106213910049](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230106213910049.png)



我们在发起一个查询部门数据的请求，此时我们可以看到在请求头中包含一个token(JWT令牌)，后续的每一次请求当中，都会将这个令牌携带到服务端。

![image-20230106214331443](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230106214331443.png)
...
![image-20230112120955145](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112120955145.png) 
...
public class MyFilterHello implements Filter {
    @Override
    public void init(FilterConfig filterConfig) throws ServletException {
        System.out.println("MyFilterHello 初始化成功了.............................................................................");
        Filter.super.init(filterConfig);
    }

    @Override
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        // 浏览器的每次请求,都会经过这里,我们可以在这里做统一的操作
        System.out.println("MyFilterHello 过滤器开始过滤.............................................................................");
        filterChain.doFilter(servletRequest, servletResponse);
    }

    @Override
    public void destroy() {
        System.out.println("MyFilterHello 销毁了.............................................................................");
...
![image-20230112121205697](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112121205697.png) 
...
![image-20230106222559935](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230106222559935.png)
...
~~~java
/*
    使用 filter 拦截除了 登录之外的所有请求,检查是否有令牌,然后决定如何处理
 */
@WebFilter(urlPatterns = "/*") // 拦截所有请求
public class MyLoginFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
       /*
        ...
        2: 判断,如果是去登录的请求,则放行
        3. 从请求头中获取请求的令牌
        4. 判断令牌是否为空,为空则提示用户登录,
        5: 不为空则从令牌中获取用户的信息 (id和name),并使用ThreadLocal进行保存
        6: 放行
        */
        HttpServletRequest request = (HttpServletRequest) servletRequest;
        String path = request.getRequestURI(); // 获取请求路径  /xx
        ...
        if(token != null){
            try {
                ...
                // 当令牌伪造或令牌过期,都会导致上面一行出现异常  自己处理异常
                Integer i = claims.get("id", Integer.class);
                ...
                // 放行
                filterChain.doFilter(servletRequest, servletResponse);
                // 这里的代码会在 controller,service,mapper执行后,继续执行
        ...
        // 给浏览器设置 响应头(告诉浏览器,我们响应的字符串是 json格式)
...
![image-20230106224322625](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230106224322625.png)

```java
public class MyThreadLocalUtils {
    private static ThreadLocal<Emp> threadLocal = new ThreadLocal<>();

    public static void set(Emp emp){
        threadLocal.set(emp);
    }

    public static Emp get(){
...
![image-20230106225658525](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230106225658525.png)

![image-20230106230332510](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230106230332510.png)

测试2：访问登录请求/login，发现过滤器拦截请求

![image-20230106230520229](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230106230520229.png)
...
![image-20230106231144348](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230106231144348.png)

测试2：访问登录请求/login，发现过滤器没有拦截请求

![image-20230106231220802](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230106231220802.png)

##### 2.4.2.3 过滤器链

所谓过滤器链指的是在一个web应用程序当中，可以配置多个过滤器，多个过滤器就形成了一个过滤器链。

![image-20230107084730393](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230107084730393.png)
...
以上就是当我们在web应用当中配置了多个过滤器，形成了这样一个过滤器链以及过滤器链的执行顺序。
...
![image-20230107085552176](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230107085552176.png)
...
![image-20230107090425999](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230107090425999.png)

通过控制台日志的输出，大家发现AbcFilter先执行DemoFilter后执行，这是为什么呢？

**其实是和过滤器的类名有关系**。以注解方式配置的Filter过滤器，它的执行优先级是按时过滤器类名的自动排序确定的，**类名排名越靠前，优先级越高**。

假如我们想让DemoFilter先执行，怎么办呢？
...
![image-20230107093757050](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230107093757050.png)
...
![image-20230107095010089](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230107095010089.png)
...
![image-20230112122130564](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112122130564.png) 
...
- 测试2：先进行登录操作，再访问部门管理页面
  ...
  ![image-20230107102922550](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230107102922550.png)
...
![image-20230107105224741](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230107105224741.png)

![image-20230107105415120](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230107105415120.png)

接下来我们再来做一个测试：将拦截器中返回值改为false

使用postman，再次点击send发送请求后，没有响应数据，说明请求被拦截了没有放行

![image-20230107105815511](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230107105815511.png)
...
![image-20230107111525558](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230107111525558.png)

控制台没有输出拦截器中的日志信息，说明`/*`没有匹配到拦截路径`/emp/1` 。

![image-20230107111812963](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230107111812963.png)

##### 2.5.2.2 执行流程

拦截器的执行流程。通过执行流程，大家就能够清晰的知道过滤器与拦截器的执行时机。

![image-20230107112136151](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230107112136151.png)
...
![image-20230107113653871](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230107113653871.png)

![image-20230107114008004](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230107114008004.png)



以上就是拦截器的执行流程。通过执行流程分析，大家应该已经清楚了过滤器和拦截器之间的区别，其实它们之间的**区别主要是两点**：
...
异常处理。首先我们先来看一下系统出现异常之后会发生什么现象，再来介绍异常处理的方案。

我们打开浏览器，访问系统中的新增部门操作，系统中已经有了 "就业部" 这个部门，我们再来增加一个就业部，看看会发生什么现象。

<img src="https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112125651073.png" alt="image-20230112125651073" style="zoom: 80%;" />   

点击确定之后，窗口关闭了，页面没有任何反应，就业部也没有添加上。 而此时，大家会发现，网络请求报错了。

<img src="https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112125737863.png" alt="image-20230112125737863" style="zoom:80%;" /> 

状态码为500，表示服务器端异常，我们打开idea，来看一下，服务器端出了什么问题。

![image-20230112125826602](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112125826602.png) 
...
![image-20230112130253486](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112130253486.png) 
...
![image-20230107121909087](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230107121909087.png)
...
![image-20230107122904214](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230107122904214.png)
...
/*
    所有异常,统一处理
 */
@RestControllerAdvice  // 标识这个 类,是统一处理异常的类
public class MyAllException {
    ...
    public Result doRuntimeException(RuntimeException  e){
        e.printStackTrace();
        return Result.error("亲,您的操作有问题,再想想...");
    }
    ...
    public Result doException(Exception  e){
        e.printStackTrace();
        return Result.error("亲,服务器正在升级,过两天再来...");
    }
...
![image-20230112131232032](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112131232032.png) 

![image-20230112131135272](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/image-20230112131135272.png) 
