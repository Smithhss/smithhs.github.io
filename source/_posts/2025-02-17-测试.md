---
title: 测试
date: 2025-02-17 12:08:04
tags:
  - Java
  - JVM
categories: 编程开发
cover: 'https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250217122207.png'
---



学习笔记如下，本笔记观看视频，[点我！](https://www.bilibili.com/video/BV1yE411Z7AP)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250217122207.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250217122253.png)

我们主要学习的是 HotSpot 版本的虚拟机。

### 4、学习路线

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250217190950.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250217122622.png)
...
+   每个线程运行需要的*内存空间*，称为*虚拟机栈*
+   每个栈由多个*栈帧*（Frame）组成，对应着每次调用方法时所占用的内存
+   每个线程只能有一个*活动栈帧*，对应着当前正在执行的方法

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250217123308.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250217125546.png)

#### 2）栈内存溢出

栈帧过大、过多、或者第三方类库操作，都有可能造成栈内存溢出 

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250217130653.png)

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250217130602.png)


**报错**: java.lang.stackOverflowError ，使用 -Xss256k 指定栈内存大小！

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250217190500.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250217190718.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250217191252.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250218132957.png)

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250218133339.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250218133512.png)

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250218133611.png)

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250218133648.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/393f30b87eb0e8f738cfb3f0217c8f82.png)

#### 3）方法区内存溢出

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250220125608.png)

+   1.8 之前会导致永久代内存溢出
    +   使用 -XX:MaxPermSize=8m 指定永久代内存大小

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250220125438.png)
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250220125533.png)

+   1.8 之后会导致元空间内存溢出
    +   使用 -XX:MaxMetaspaceSize=8m 指定元空间大小

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250220125648.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250220125742.png)

然后使用 javap -v Test.class 命令反编译查看结果。  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/4dcb07eed7d6503f730bf641484f6343.png)  

每条指令都会对应常量池表中一个地址，常量池表中的地址可能对应着一个类名、方法名、
参数类型等信息。 

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/134c15382b6ebdd8b7677cfaa39a3f59.png)  
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250220125906.png)

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250228102032.png)

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250228102323.png)
...
![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250228110114.png)

![image.png](https://smith-1315833455.cos.ap-beijing.myqcloud.com/blog/20250228110507.png)
...
public class Main {
	public static void main(String[] args) {
		...
		String st2 = str.intern(); //将这个字符串对象尝试放入串池，
...
文件读写流程：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/e0d880256cc6cf449e4bbe56e63b9ec4.png)  
因为 java 不能直接操作文件管理，需要切换到内核态，使用本地方法进行操作，然后读取磁盘文件，会在系统内存中创建一个缓冲区，将数据读到系统缓冲区， 然后在将系统缓冲区数据，复制到 java 堆内存中。缺点是数据存储了两份，在系统内存中有一份，java 堆中有一份，造成了不必要的复制。

**使用了 DirectBuffer 文件读取流程**  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/55798c5ea77cfadb11e3090dcb2d1d91.png)  
...
这个引用计数法听起来不错，但是有一个弊端，如下图所示，循环引用时，两个对象的计数都为1，导致两个对象都无法被释放。![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/90e292683056f7f6855e001f3fd6074d.png)
...
第一步：  
使用 jps 命令，查看程序的进程  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/e7cddab049d48c37997f272e6c549f86.png)  
第二步：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/0dc57051a649cb940a1bd0000093f3d0.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/699707785fe723031e3fbba7d65500ff.png)  
分析的 gc root，找到了 ArrayList 对象，然后将 list 置为null，再次转储，那么 list 对象就会被回收。

#### 3）四种引用

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/040b7919911ef4054ad44d28a03d0ecb.png)
...
7.  弱引用（WeakReference）  
    ...
    可以配合引用队列来释放弱引用自身
8.  虚引用（PhantomReference）  
    必须配合引用队列使用，主要配合 ByteBuffer 使用，被引用对象回收时，会将虚引用入队，  
    由 Reference Handler 线程调用虚引用相关方法释放直接内存
9.  终结器引用（FinalReference）  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/0ad6e438e1ee6d648ade35e95f675224.png)  
method2 方法解析：  
在 list 集合中存放了 软引用对象，当内存不足时，会触发 full gc，将软引用的对象回收。细节如图：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/d94c7e823625786a15c37ec8b3bd9234.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/5653f0387186412bca46e639195f7214.png)  
**弱引用演示**
...
+   速度较快
+   会产生内存碎片  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/b21e5a0cb4e46fa6898b629083cff5a5.png)
...
+   速度慢
+   没有内存碎片  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/3b19bc99ca047e25a333071203efca68.png)
...
+   不会有内存碎片
+   需要占用两倍内存空间  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/76d7d89f2853950c27c4501aa907a61a.png)

### 3、分代垃圾回收

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f896d0c61f349e3e976b265e6679a1bd.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/17c76f678a080178a048bff9d1c77734.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/1b27ce8a87378ccbc9f73cb943383d1a.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/67ec4a2259b0f4345949b65f9b4898d3.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/9087899f384668837904dcfdccfe44de.png)  
...
E：eden，S：幸存区，O：老年代  
新生代收集会产生 STW ！  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/d383f88a30443cab3b5513029d8ff85f.gif)

##### Young Collection + CM

在 Young GC 时会进行 GC Root 的初始化标记  
老年代占用堆空间比例达到阈值时，进行并发标记（不会STW），由下面的 JVM 参数决定 -XX:InitiatingHeapOccupancyPercent=percent （默认45%）  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/7008b25bb405e3c1c59c76084b108cae.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/a395a00fc6676cc60d1c59f5f22fb9e1.png)
...
+   新生代回收的跨代引用（老年代引用新生代）问题  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/85a8d5e8cadf7156dd0542eb06e76ce4.png)
+   卡表 与 Remembered Set
    +   Remembered Set 存在于E中，用于保存新生代对象对应的脏卡
        +   脏卡：O 被划分为多个区域（一个区域512K），如果该区域引用了新生代对象，则该区域被称为脏卡
+   在引用变更时通过 post-write barried + dirty card queue
+   concurrent refinement threads 更新 Remembered Set  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/0ded8a56865c7aadd79680a622b7cfb0.png)
...
+   灰色：正在处理中的
+   白色：还未处理的  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/61ede1c6b91eea437a7138c065cd2b79.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/ff08a3049c63636df88950ac8dd2c5c9.png)  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/8d3c7095fad10df8f0deabe677f81183.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/88312624aeb062a3209bc2fa7b9e951c.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/4736f9f9ad3c91252ff22cfbf1c6bd06.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/6375e6b47e25207e01d584a36a67c3e3.png)  
**方法字节码载入方法区**  
（stack=2，locals=4） 对应操作数栈有 2 个空间（每个空间 4 个字节），局部变量表中有 4 个槽位。  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/cb5c808d5c00d0d5d7110fd9deac6511.png)  
**执行引擎开始执行字节码**  
**bipush 10**
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/84935083c058c735b6e33914540dc81c.png)  
**istore 1**  
将操作数栈栈顶元素弹出，放入局部变量表的 slot 1 中  
对应代码中的 a = 10  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/42caf2f7078e4835976069697852bd9f.png)  
**ldc #3**  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/3616501ab0db38fe46ff30b985a1e722.png)  
**istore 2**  
将操作数栈中的元素弹出，放到局部变量表的 2 号位置  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/5d68c9bf80b9153319c26ec853339214.png)  
**iload1 iload2**  
将局部变量表中 1 号位置和 2 号位置的元素放入操作数栈中。因为只能在操作数栈中执行运算操作  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/d9650c22a246912f5cfe5fefb53773d0.png)  
**iadd**  
将操作数栈中的两个元素弹出栈并相加，结果在压入操作数栈中。  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/415728aad9e8a1e49805ddf19d6ef8f5.png)  
**istore 3**  
将操作数栈中的元素弹出，放入局部变量表的3号位置。  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/804b87e0a61c5be5b3eedd4b36782107.png)  
**getstatic #4**  
在运行时常量池中找到 #4 ，发现是一个对象，在堆内存中找到该对象，并将其引用放入操作数栈中  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/8e1156a466ed884934765428431e3f70.png)  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/03d579cb62c2ef78fa6d5f01442f115e.png)  
**iload 3**  
将局部变量表中 3 号位置的元素压入操作数栈中。  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/0e229d9b0bf98f97fc9d64b3dffb857d.png)  
**invokevirtual #5**  
找到常量池 #5 项，定位到方法区 java/io/PrintStream.println:(I)V 方法  
生成新的栈帧（分配 locals、stack等）  
传递参数，执行新栈帧中的字节码  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/5a0528691b05a8a505242f8b27ef72c1.png)  
执行完毕，弹出栈帧  
清除 main 操作数栈内容  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/462796df7cb5103ccf81024893825e25.png)  
**return**  
...
+   如果这个类还有父类没有加载，先加载父类
+   加载和链接可能是交替运行的  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/2946c0ce3f66277be06d5fc1ee2fb869.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/bb509bd9b351fc0193657649e4e8ead7.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/191f81734faed8472d18b8321f8ccec6.png)
