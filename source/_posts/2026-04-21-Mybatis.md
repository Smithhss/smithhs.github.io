...
![image-20220901114951631](img/image-20220901114951631.png) 
...
![image-20221209155704203](img/image-20221209155704203.png) 

图形化工具会把数据库执行的查询结果，使用表格的形式展现出来

![image-20220901121116813](img/image-20220901121116813.png) 

现在使用Mybatis操作数据库，就是在Mybatis中编写SQL查询代码，发送给数据库执行，数据库执行后返回结果。

![image-20221209155904370](img/image-20221209155904370.png)

 Mybatis会把数据库执行的查询结果，使用实体类封装起来（一行记录对应一个实体类对象）

![image-20221209161623051](img/image-20221209161623051.png)
...
![image-20221209162827242](img/image-20221209162827242.png)

![image-20221209163123443](img/image-20221209163123443.png)
...
![image-20221209170354143](img/image-20221209170354143.png)

#### 1.2.2 配置Mybatis

> 在之前使用图形化客户端工具，连接MySQL数据库时，需要配置：
>
> ![image-20221209172527630](img/image-20221209172527630.png)
>
> 连接数据库的四大参数：
>
> - MySQL驱动类 
...
spring:
  datasource:
    ...
    url: jdbc:mysql://localhost:3306/day15?useUnicode=true&characterEncoding=UTF-8&useSSL=false&serverTimezone=Asia/Shanghai&rewriteBatchedStatements=true&allowPublicKeyRetrieval=true
    username: root
    password: root
...
mybatis-plus:
  ...
  type-aliases-package: com.zhisheng.orders.entity
  global-config:
    datacenter-id: 1
    workerId: 3
  configuration:
...
![image-20221209175843651](img/image-20221209175843651.png)

UserMapper：

~~~java
import com.kge.pojo.User;
...
![image-20221210143348119](img/image-20221210143348119.png)

配置完成之后，发现SQL语句中的关键字有提示了，但还存在不识别表名(列名)的情况：

![image-20221210143934318](img/image-20221210143934318.png)

> 产生原因：Idea和数据库没有建立连接，不识别表信息
>
> 解决方案：在Idea中配置MySQL数据库连接

![image-20221210144139792](img/image-20221210144139792.png) 
...
![image-20221210164641266](img/image-20221210164641266.png)
...
>
> ![image-20221210170733921](img/image-20221210170733921.png)
...
![image-20221210165506359](img/image-20221210165506359.png)
...
![image-20221210180155700](img/image-20221210180155700.png) 

![image-20221210180343288](img/image-20221210180343288.png)

![image-20221210180515206](img/image-20221210180515206.png)
...
![image-20221210182008131](img/image-20221210182008131.png)
...
![image-20221210182500817](img/image-20221210182500817.png)
...
![image-20221210183336095](img/image-20221210183336095.png)
...
![image-20220901164225644](img/image-20220901164225644.png) 
...
![image-20221210202222206](img/image-20221210202222206.png)
...
![image-20221210205419634](img/image-20221210205419634.png)

第1步：进入到DOS

![image-20221211124744203](img/image-20221211124744203.png)

![image-20221211124840720](img/image-20221211124840720.png)
...
![image-20221210211605231](img/image-20221210211605231.png)

第3步：打开浏览器输入`http://localhost:9090/login.html`

![image-20221210212406527](img/image-20221210212406527.png)

发现竟然能够登录成功：

![image-20221210212511915](img/image-20221210212511915.png)
...
![image-20221210213311518](img/image-20221210213311518.png)

> ![image-20221210214431228](img/image-20221210214431228.png)
>
...
![image-20221211125751981](img/image-20221211125751981.png)

以上操作SQL语句的执行：

![image-20221211130011973](img/image-20221211130011973.png)
...
![image-20221211134239610](img/image-20221211134239610.png)

### 4.1 基本新增

员工表结构：

![image-20221211134746319](img/image-20221211134746319.png)
...
```java
import com.kge.mapper.EmpMapper;
import com.kge.pojo.Emp;
...
> 日志输出：
>
> ![image-20221211140222240](img/image-20221211140222240.png)
...
>
> ![image-20221211150353385](img/image-20221211150353385.png)
>
> 业务场景：在前面讲解到的苍穹外卖菜品与套餐模块的表结构，菜品与套餐是多对多的关系，一个套餐对应多个菜品。既然是多对多的关系，是不是有一张套餐菜品中间表来维护它们之间的关系。
>
> ![image-20221212093655389](img/image-20221212093655389.png)
>
...
![image-20221212095605863](img/image-20221212095605863.png)
...
![image-20221212101331292](img/image-20221212101331292.png) 
...
> 执行结果：
>
> ![image-20221212103004961](img/image-20221212103004961.png)
>
...
![image-20221212103124490](img/image-20221212103124490.png)
...
> 再次执行测试类：
>
> ![image-20221212111027396](img/image-20221212111027396.png)
...
![image-20221212113422924](img/image-20221212113422924.png)
...
> ![image-20221212115149151](img/image-20221212115149151.png)
>
> 以上方式注意事项：
>
...
>
> ![image-20221212120006242](img/image-20221212120006242.png)
...
>
> ![image-20221212150611796](img/image-20221212150611796.png)
...
<!DOCTYPE mapper
        PUBLIC "-//mybatis.org//DTD Mapper 3.0//EN"
        "https://mybatis.org/dtd/mybatis-3-mapper.dtd">
<mapper namespace="com.kge.mapper.EmpMapper">
...
<mapper namespace="com.kge.mapper.EmpMapper">
    ...
    <!--查询操作-->
    <select id="list" resultType="com.kge.pojo.Emp">
        select * from emp
        where name like concat('%',#{name},'%')
              and gender = #{gender}
              and entrydate between #{begin} and #{end}
...
> 运行测试类，执行结果：
>
> ![image-20221212163719534](img/image-20221212163719534.png)
...
![image-20221213120923252](img/image-20221213120923252.png)
...
>
> ![image-20220901173948645](img/image-20220901173948645.png) 
...
![](img/image-20220901172933012.png)

![image-20220901173203491](img/image-20220901173203491.png)

而在我们刚才编写的SQL语句中，我们会看到，我们将三个条件直接写死了。 如果页面只传递了参数姓名name 字段，其他两个字段 性别 和 入职时间没有传递，那么这两个参数的值就是null。

此时，执行的SQL语句为：

![image-20220901173431554](img/image-20220901173431554.png) 
...
> 执行的SQL语句： 
>
> ![image-20221213140353285](img/image-20221213140353285.png)
...
![image-20221213141139015](img/image-20221213141139015.png) 

![image-20221213141253355](img/image-20221213141253355.png) 
...
![image-20221213143854434](img/image-20221213143854434.png)
...
> 执行的SQL语句：
>
> ![image-20221213141909455](img/image-20221213141909455.png)
...
> 执行的SQL语句：
>
> ![image-20221213152533851](img/image-20221213152533851.png)
...
> 执行的SQL语句：
>
> ![image-20221213152850322](img/image-20221213152850322.png)
...
> 再次执行测试方法，执行的SQL语句：
>
> ![image-20221213153329553](img/image-20221213153329553.png)
...
![image-20220901181751004](img/image-20220901181751004.png) 
...
> ![image-20221213165710141](img/image-20221213165710141.png)

> 执行的SQL语句：
>
> ![image-20221213164957636](img/image-20221213164957636.png)



## 8.4 动态SQL-sql&include
