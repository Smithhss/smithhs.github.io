...
**并发编程已完结，章节如下：**  
[Java 并发编程上篇 -（Synchronized 原理、LockSupport 原理、ReentrantLock 原理）](https://blog.csdn.net/weixin_50280576/article/details/113033975)  
...
[学习视频](https://www.bilibili.com/video/BV16J411h7Rd)
...
2.  调用 sleep 会让当前线程从 Running 进入 Timed Waiting 状态（阻塞），可通过state()方法查看
...
6.  调用 yield 会让当前线程从 Running 进入 Runnable 就绪状态（仍然有可能被执行），然后调度执行其它线程
7.  具体的实现依赖于操作系统的任务调度器
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/76d25868b2c84bf4d8d160b3dad50b14.png)  
代码实现如下：
...
从操作系统层划分，线程有 5 种状态  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/370df172999874ee6a837656163ba2ba.png)

8.  初始状态，仅仅是在语言层面上创建了线程对象，即Thead thread = new Thead();，还未与操作系统线程关联
9.  可运行状态，也称就绪状态，指该线程已经被创建，与操作系统相关联，等待cpu给它分配时间片就可运行
10.  运行状态，指线程获取了CPU时间片，正在运行  
...
12.  终止状态，表示线程已经执行完毕，生命周期已经结束，不会再转换为其它状态

#### 2）线程的 6 种状态

这是从 Java API 层面来描述的，我们主要研究的就是这种。可以参考文章，[点这里](https://blog.csdn.net/pange1991/article/details/53860651)  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/97313409e2f6ffa8a921cbad3ac9ff58.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/c4695c615206e84ee817184201a4de03.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/7a7c02e83afbff2ca0cd0142572d9de2.png)  
...
普通对象  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/395ed127e00bf6e0c2d8f559bee80239.png)  
数组对象  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/d3460fbf508de170f0e27ed247f33fff.png)  
其中 Mark Word 结构为  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/01a50cb96be8df4a08db0bdc7b6b9298.png)  
所以一个对象的结构如下：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/99e3b65399c202fdd9491824f9eae656.png)

#### 2）Monitor 原理

Monitor 被翻译为监视器或者说管程  
每个 java 对象都可以关联一个 Monitor ，如果使用 synchronized 给对象上锁（重量级），该对象头的 Mark Word 中就被设置为指向 Monitor 对象的指针。  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/4ad9715a08687e2577511225da85cec4.png)
...
13.  每次指向到 synchronized 代码块时，都会创建锁记录（Lock Record）对象，每个线程都会包括一个锁记录的结构，锁记录内部可以储存对象的 Mark Word 和对象引用 reference  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/add15e8a40b3efaa53bbcbb831d450c7.png)
14.  让锁记录中的 Object reference 指向对象，并且尝试用 cas(compare and sweep) 替换 Object 对象的 Mark Word ，将 Mark Word 的值存入锁记录中。  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/810c72b2e6995cf2aa2976d81bedd949.png)
15.  如果 cas 替换成功，那么对象的对象头储存的就是锁记录的地址和状态 00 表示轻量级锁，如下所示  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/280de3bf59023c5361807e1f6dcb7d39.png)
16.  如果cas失败，有两种情况
    ...
    2.  如果是自己的线程已经执行了 synchronized 进行加锁，那么再添加一条 Lock Record 作为重入的计数。  
        ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/36361df0cd3f7b4d3a166b8bc0e6ca97.png)
17.  当线程退出 synchronized 代码块的时候，如果获取的是取值为 null 的锁记录，表示有重入，这时重置锁记录，表示重入计数减一  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/280de3bf59023c5361807e1f6dcb7d39.png)
18.  当线程退出 synchronized 代码块的时候，如果获取的锁记录取值不为 null，那么使用 cas 将 Mark Word 的值恢复给对象
    1.  成功则解锁成功
...
19.  当 Thread-1 进行轻量级加锁时，Thread-0 已经对该对象加了轻量级锁  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/dc03e1d436c432949f02f9ce568b24dc.png)
20.  这时 Thread-1 加轻量级锁失败，进入锁膨胀流程，
    +   即为对象申请Monitor锁，让Object指向重量级锁地址
    +   然后自己进入Monitor 的EntryList 变成BLOCKED状态  
        ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/2d76ab27fadeaac2555b3677a5de0b89.png)
21.  当 Thread-0 退出 synchronized 同步块时，使用 cas 将 Mark Word 的值恢复给对象头，对象的对象头指向 Monitor，那么会进入重量级锁的解锁过程，即按照 Monitor 的地址找到 Monitor 对象，将 Owner 设置为 null ，唤醒 EntryList 中的 Thread-1 线程
...
22.  自旋重试成功的情况  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/bf48296b7a75af6f26d0c389483e2cc5.png)
23.  自旋重试失败的情况，自旋了一定次数还是没有等到持锁的线程释放锁  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/fba5e63a6ceda99a0b36dcf60d4f98ca.png)
...
分析如图：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/355b998c6cf4d8b03f7f0da95d19d858.png)

##### 偏向状态

对象头格式如下：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/b0e2a8f17fd11e21e47547beccd6b487.png)  
一个对象的创建过程
...
34.  旋锁，若自旋失败，那么锁就会转化为重量级锁，在这种情况下，无法获取到锁的线程都会进入到 moniter(即内核态)，自旋最大的特点是避免了线程从用户态进入到内核态。

### 6、Wait/Notify

#### 1）原理

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/422cb92431a58006a0ea13b10064da45.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/3acddfe65eab379f7551e53b22bffb5b.png)  
代码如下：
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/b26ff9b5c8c7a73682caed98b5e977e8.png)

#### 小结

35.  当调用 wait 时，首先需要确保调用了 wait 方法的线程已经持有了对象的锁(调用 wait 方法的代码片段需要放在 sychronized 块或者时 sychronized 方法中，这样才可以确保线程在调用wait方法前已经获取到了对象的锁)
36.  当调用 wait 时，该线程就会释放掉这个对象的锁，然后进入等待状态 (wait set)
37.  当线程调用了 wait 后进入到等待状态时，它就可以等待其他线程调用相同对象的 notify 或者 notifyAll 方法使得自己被唤醒
38.  一旦这个线程被其它线程唤醒之后，该线程就会与其它线程以同开始竞争这个对象的锁(公平竞争)；只有当该线程获取到对象的锁后，线程才会继续往下执行
39.  当调用对象的 notify 方法时，他会随机唤醒对象等待集合 (wait set) 中的任意一个线程，当某个线程被唤醒后，它就会与其它线程一同竞争对象的锁
40.  当调用对象的 notifyAll 方法时，它会唤醒该对象等待集合 (wait set) 中的所有线程，这些线程被唤醒后，又会开始竞争对象的锁
41.  在某一时刻，只有唯一的一个线程能拥有对象的锁
...
42.  先调用 park
    ...
    3.  线程进入 \_cond 条件变量阻塞
    4.  设置 \_counter = 0  
        ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/fcfeec4840a2d7f675b95f30f18f35d4.png)
43.  调用 upark
    1.  调用 Unsafe.unpark(Thread\_0) 方法，设置 \_counter 为 1
    2.  唤醒 \_cond 条件变量中的 Thread\_0
    3.  Thread\_0 恢复运行
    4.  设置 \_counter 为 0  
        ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/7d3c2f5ff6efa1617e2220245d0af351.png)

**先调用upark再调用park的过程**

44.  调用 Unsafe.unpark(Thread\_0) 方法，设置 \_counter 为 1
45.  当前线程调用 Unsafe.park() 方法
46.  检查 \_counter ，本情况为 1，这时线程无需阻塞，继续运行
47.  设置 \_counter 为 0  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/d84ac2cbb48202a5eb2b01ddbacf6806.png)

### 8、线程状态转换

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/b8357fb08692d180e5e0a6f6f8e079c6.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/c92fcdf72ba0b7bfb006d61739a88fa2.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f097b5788946f4ce5bf8e3453eab8e00.png)  
**解决方法**
...
分析如图：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/91849aa485ea9cc09aa5571651b2f1b8.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/dcbcdc8bb7f67e1a529dfb078bdff8ff.png)  
...
4.  可见性 - 由 JVM 缓存优化引起
5.  有序性 - 由 JVM 指令重排序优化引起
6.  happens-before 规则
7.  原理方面
    1.  volatile
8.  模式方面
    2.  两阶段终止模式的 volatile 改进
    3.  同步模式之 balking
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/e9858994dd2ef7ebadfc6b098ad995de.png)  
如图所示，它的工作流程如下：  
...
得从缓存说起，缓存与内存的速度比较  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/a8b0be45afea945daaf5114139b8518d.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/cbd4038fbbba5dc6030effa873cb5ab4.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/93db8f3d2b65086ae967b458e2fabb82.png)
...
**第一步：add 方法分析，流程图如下**  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/acf18e7cbf1bb6f97e21549a51d69731.png)  
源码如下：
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/e121a002ea3c5c7bb7cd9f5bf979fe10.png)  
源码如下：
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/1130eac3501812c47e7cac07f5864103.png)  
...
jdk 线程池的拒绝策略结构图如下：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/38e15d7e65562b50325f25fc47d79e50.png)  
...
+   核心线程数是 0， 最大线程数是 Integer.MAX\_VALUE，救急线程的空闲生存时间是 60s，意味着
...
6.  希望多个任务排队执行。线程数固定为 1，任务数多于 1 时，会放入无界队列排队。任务执行完毕，这唯一的线程也不会被释放。
7.  newSingleThreadExecutor 和 newFixedThreadPool 区别：
...
8.  实际使用中需要根据自己机器的性能、业务场景来手动配置线程池的参数比如核心线程数、使用的任务队列、饱和策略等等。
9.  我们应该显示地给我们的线程池命名，这样有助于我们定位问题。
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/d3c879056004c73eadbc8a48b24787ab.png)
...
**Connector 配置如下：**  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/d33c7e4ca54bc49989db5e04f4e84b7a.png)**Executor 线程池配置如下：**  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/cee20644351c3fd90ca1bb1bf7fdc1b1.png)  
可以看到该线程池实现的是一个无界的队列，所以说是不是执行任务的线程数大于了核心线程数，都会添加到阻塞队列中，那么救急线程是不是就不会用到呢，其实不是，分析如下图：![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/001435641e41063d4a434ca4d5c7f66a.png)  
...
10.  两个工人是同一个线程池中的两个线程
11.  他们要做的事情是：为客人点餐和到后厨做菜，这是两个阶段的工作
    1.  客人点餐：必须先点完餐，等菜做好，上菜，在此期间处理点餐的工人必须等待
    2.  后厨做菜：没啥说的，做就是了
12.  比如工人 A 处理了点餐任务，接下来它要等着 工人 B 把菜做好，然后上菜，他俩也配合的蛮好 但现在同时来了两个客人，这个时候工人 A 和工人 B 都去处理点餐了，这时没人做饭了，这就是饥饿。
...
13.  CPU 密集型运算 通常采用 cpu 核数 + 1 能够实现最优的 CPU 利用率，+1 是保证当线程由于页缺失故障（操作系统）或其它原因导致暂停时，额外的这个线程就能顶上去，保证 CPU 时钟周期不被浪费
14.  I/O 密集型运算 CPU 不总是处于繁忙状态，例如，当你执行业务计算时，这时候会使用 CPU 资源，但当你执行 I/O 操作时、远程 RPC 调用时，包括进行数据库操作时，这时候 CPU 就闲下来了，你可以利用多线程提高它的利用率。
    1.  经验公式如下：
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/a16094a3b11c0c4587a1d3c83c54d0c9.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/3be57cfdfe3308eea25375678719c5d2.png)  
第一个竞争出现时
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/1b1338b1997a88ad90dab943a5325e8b.png)  
Thread-1 执行了
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/90e11c582c5f5c2f67e13f4f42ea3ed8.png)  
当前线程进入 acquire方法的 acquireQueued 逻辑

4.  acquireQueued 会在一个死循环中不断尝试获得锁，失败后进入 park 阻塞
5.  如果自己是紧邻着 head（排第二位），那么再次 tryAcquire 尝试获取锁，我们这里设置这时 state 仍为 1，失败
6.  进入 shouldParkAfterFailedAcquire 逻辑，将前驱 node，即 head 的 waitStatus 改为 -1，这次返回 false  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/88d75f973498aade900c20fd281af9a6.png)
...
9.  进入 parkAndCheckInterrupt， Thread-1 park（灰色表示已经阻塞）  
    ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/98d5bfa3f430f6c86e194f8c6a7d3ca7.png)

再从次有多个线程经历上述过程竞争失败，变成这个样子  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/106b024819b64a2e6f4359c306fd96cf.png)  
Thread-0 释放锁，进入 tryRelease 流程，如果成功

+   设置 exclusiveOwnerThread 为 null
+   state = 0

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/6ac5346bf9c47fde1cd0b3ea0c454122.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f8af2886db51785cd88e117e628a1a77.png)  
如果加锁成功（没有竞争），会设置 （acquireQueued 方法中）

10.  exclusiveOwnerThread 为 Thread-1，state = 1
11.  head 指向刚刚 Thread-1 所在的 Node，该 Node 清空 Thread
12.  原本的 head 因为从链表断开，而可被垃圾回收

如果这时候有其它线程来竞争（非公平的体现），例如这时有 Thread-4 来了  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/efb0796b02d5dab3dc42057b53dd75c7.png)  
如果不巧又被 Thread-4 占了先

13.  Thread-4 被设置为 exclusiveOwnerThread，state = 1
14.  Thread-1 再次进入 acquireQueued 流程，获取锁失败，重新进入 park 阻塞
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/47a3542383a7ccbd6c569671c603a01d.png)  
接下来进入 AQS 的 fullyRelease 流程，释放同步器上的锁  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/64f793849fadd8c578b4ee2529fd79b4.png)  
unpark AQS 队列中的下一个节点，竞争锁，假设没有其他竞争线程，那么 Thread-1 竞争成功  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/9b037ce71d9d78f140677c3a88a9d8c7.png)  
park 阻塞 Thread-0  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/e21d6eb112f1a88d42ee3f437391ca99.png)  
**signal 流程**  
假设 Thread-1 要来唤醒 Thread-0  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/a60642397ac8c296c8396af4fed4fb0e.png)  
进入 ConditionObject 的 doSignal 流程，取得等待队列中第一个 Node，即 Thread-0 所在 Node  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/5d2fde664589e81f9f83c76f07eaaf2a.png)  
执行 transferForSignal 流程，将该 Node 加入 AQS 队列尾部，将 Thread-0 的 waitStatus 改为 0，Thread-3 的waitStatus 改为 -1  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/c4738fb6f83f79c511e3dfd53db45fee.png)  
...
### [读写锁](https://so.csdn.net/so/search?q=%E8%AF%BB%E5%86%99%E9%94%81&spm=1001.2101.3001.7020)原理
...
15.  读锁不支持条件变量
16.  重入时升级不支持：即持有读锁的情况下去获取写锁，会导致获取写锁永久等待
...
更新时，是先清缓存还是先更新数据库？  
先清除缓存操作如下：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/4fcf88000fbfa43c1c97041304f1600d.png)

先更新数据库操作如下：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/72762c0b9e2e8a9737192ad05f81b7e1.png)  
补充一种情况，假设查询线程 A 查询数据时恰好缓存数据由于时间到期失效，或是第一次查询：这种情况的出现几率非常小：  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/ef1d80d18f22f2677ed6e4c2f1b7d684.png)  
实现代码如下：
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/cdc0eca4bf613be9ac99ee67a8ba3273.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/a42c36fcb8076bab830f8288b71b3e9a.png)  
3）这时会进入 sync.doAcquireShared(1) 流程，首先也是调用 addWaiter 添加节点，不同之处在于节点被设置为 Node.SHARED 模式而非 Node.EXCLUSIVE 模式，注意此时 t2 仍处于活跃状态  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/80e90a7a7d172c85be95fc8be20e5554.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/67c5fadbeff539aea6450719557f42e6.png)

**又继续执行**：t3 r.lock，t4 w.lock  
这种状态下，假设又有 t3 加读锁和 t4 加写锁，这期间 t1 仍然持有锁，就变成了下面的样子  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/d324c652f6795e022344a0312159dde5.png)  
**继续执行 t1 w.unlock**  
这时会走到写锁的 sync.release(1) 流程，调用 sync.tryRelease(1) 成功，变成下面的样子  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/0061a0419492f8605453b2697b20312b.png)  
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/90aa60c77f7c7d3851fe05e1dbbc149e.png)  
这时 t2 已经恢复运行，接下来 t2 调用 setHeadAndPropagate(node, 1)，它原本所在节点被置为头节点  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/e26e889cff68ef7457de6fcacdb86fc1.png)  
事情还没完，在 setHeadAndPropagate 方法内还会检查下一个节点是否是 shared，如果是则调用 doReleaseShared() 将 head 的状态从 -1 改为 0 并唤醒老二，这时 t3 在 doAcquireShared 内 parkAndCheckInterrupt() 处恢复运行.  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/edaf20d10c6d2700a1de4f3dc2247474.png)  
这回再来一次 for (;; ) 执行 tryAcquireShared 成功则让读锁计数加一  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/6e8616c5b58760f77ff09dbab38057fa.png)  
这时 t3 已经恢复运行，接下来 t3 调用 setHeadAndPropagate(node, 1)，它原本所在节点被置为头节点  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/657d5849927bb5ab6b1532968ace0d6a.png)  
再继续执行t2 r.unlock，t3 r.unlock t2  
进入 sync.releaseShared(1) 中，调用 tryReleaseShared(1) 让计数减一，但由于计数还不为零  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/3daed09ed04bf0a2b3d72086e217ba27.png)  
t3 进入 sync.releaseShared(1) 中，调用 tryReleaseShared(1) 让计数减一，这回计数为零了，进入 doReleaseShared() 将头节点从 -1 改为 0 并唤醒老二，即  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/584df6b3b02d9e28de3fa83d4943a7c9.png)  
之后 t4 在 acquireQueued 中 parkAndCheckInterrupt 处恢复运行，再次 for (;; ) 这次自己是老二，并且没有其他 竞争，tryAcquire(1) 成功，修改头结点，流程结束  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f2b772555394848d7d70fc415d89677a.png)  
源码分析：  
写锁上锁流程
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/688a51632dbad6a5c09b4f3c8a296590.png)  
假设其中 Thread-1，Thread-2，Thread-4 cas 竞争成功，而 Thread-0 和 Thread-3 竞争失败，进入 AQS 队列park 阻塞  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/0d024f1e95afd66a48504f7f0c3a7c5d.png)  
这时 Thread-4 释放了 permits，状态如下  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/970bd000b485fe1ac37cbcdc3481f346.png)  
接下来 Thread-0 竞争成功，permits 再次设置为 0，设置自己为 head 节点，断开原来的 head 节点，unpark 接下来的 Thread-3 节点，但由于 permits 是 0，因此 Thread-3 在尝试不成功后再次进入 park 状态  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/c7aae46c12e1932a6b101378a9208c8d.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/947f0f6dae9597b653f8f95742beb960.png)  
当一个节点入队 last = last.next = node;  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/bdd0151996134309712bffa16261649c.png)  
再来一个节点入队 last = last.next = node;  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/37ac679672e0c5a3f5e7e7971430df65.png)
...
h = head;  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/97e3704de5a3c74d602e5f59d9cd3d69.png)  
first = h.next;  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/c583387ec40e515dd50f080822035c58.png)  
h.next = h;  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/2a4e01be49ebc0411451f69f8875a8f2.png)  
head = first;  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/83c018af4edc7734231e6bd387579313.png)
