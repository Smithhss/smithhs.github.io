...
public class MyAspect1 {
    //前置通知
    @Before("execution(* com.kge.service.*.*(..))")
    ...
    //后置通知
    @After("execution(* com.kge.service.*.*(..))")
    public void after(JoinPoint joinPoint){
        log.info("after ...");
    }

    //返回后通知（程序在正常执行的情况下，会执行的后置通知）
    @AfterReturning("execution(* com.kge.service.*.*(..))")
    public void afterReturning(JoinPoint joinPoint){
        log.info("afterReturning ...");
    }

    //异常通知（程序在出现异常的情况下，执行的后置通知）
    @AfterThrowing("execution(* com.kge.service.*.*(..))")
...
public class MyAspect2 {
    ...
    @Before("com.kge.aspect.MyAspect1.pt()")
    public void before(){
...
~~~java
@Before("execution(void com.kge.service.impl.DeptServiceImpl.delete(java.lang.Integer))")
~~~
...
- 使用 `*` 代替参数
  ...
  ```java
  execution(* com.kge.service.impl.DeptServiceImpl.delete(*))
  ```
...
- 描述切入点方法通常基于接口描述，而不是直接描述实现类，增强拓展性
  ...
  ~~~java
  execution(* com.kge.service.DeptService.*(..))
  ~~~

- 在满足业务需要的前提下，尽量缩小切入点的匹配范围。如：包名匹配尽量不使用 ..，使用 * 匹配单个包

  ~~~java
  execution(* com.kge.*.*.DeptServiceImpl.find*(..))
  ~~~
...
public class MyAspect6 {
    ...
    //后置通知
    @After("@annotation(com.kge.anno.MyLog)")
    public void after(){
...
public class MyAspect7 {
    ...
    @Pointcut("@annotation(com.kge.anno.MyLog)")
    private void pt(){}
